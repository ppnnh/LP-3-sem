...
namespace In
{
	IN getin(wchar_t infile[], wchar_t ITfile[], wchar_t LTfile[], LT::LexTable& lexTable, IT::IdTable& idTable, PN::Data& PN)
	{
		...
		for (; i < fulltext_size; ++i)
		{
			...
			default:
				if (tmp.size() != 0)
				{
					if (Analysis::TokenAnalysis(Functions::stringToChar(tmp), lexTable, idTable, curCol - 1, curLine, PN))
					{
						tmp.clear();
						curCol++;
						i--;
